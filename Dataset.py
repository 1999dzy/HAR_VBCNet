...
def UT_HAR_dataset(root_dir):
    ...
    for data_dir in data_list:
        ...
        with open(data_dir, 'rb') as f:
            data = np.load(f)  # ´ò¿ª±£´æf
            # print(data)
            # print(len(data)) 500 3977 496
            data = data.reshape(len(data), 1, 250, 90)
            # print(len(data)) 500
            # print(data)
            data_norm = (data - np.min(data)) / (np.max(data) - np.min(data))  # 0-1
...
root_dir = 'D:\\WIFIData\\data1'
UT_HAR_dataset(root_dir)
